...
namespace Buttplug4Net35
{
    ...
    public class ButtplugWSClient
    {
        ...
        private void OnPingTimer(object aState)
        {
            try
            {
                var msg = SendMessage(new Ping()).Result;
                if (msg is Error)
                {
                    _owningDispatcher.Send(_ =>
                    {
        ...
        {
            return SendMessageExpectOk(new StartScanning());
        }
        ...
        /// </returns>
        public Task<bool> StopScanning()
        {
            return SendMessageExpectOk(new StopScanning());
        }
        ...
        /// cref="ErrorReceived"/> event will be triggered.
        /// </returns>
        public Task<bool> RequestLog(string aLogLevel)
        {
            return SendMessageExpectOk(new RequestLog(aLogLevel));
        }
        ...
        protected Task<ButtplugMessage> SendMessage(ButtplugMessage aMsg)
        {
            _bpLogger.Trace("Pre-send message");
            var t = new Task<ButtplugMessage>(() =>
            {
                _bpLogger.Trace("Sending message");
        ...
        {
            return _parser.Serialize(aMsgs, 1);
        }
